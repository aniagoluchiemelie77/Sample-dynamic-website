    ...
    <section class="sectioneer">
        <div class="posts_div1 postsdiv sectioneer_divcontainer">
            <div class="page_links">
                <a href="<?php echo $base_url . 'admin_homepage.php'; ?>"><?php echo $translations['home']; ?></a> > <p><?php echo $translations['posts']; ?></p> > <p><?php echo $translations['commentaries']; ?></p>
            </div>
            <div class="posts_header">
                <h1><?php echo $translations['commentaries']; ?></h1>
            </div>
            <div class="posts_divcontainer border-gradient-side-dark">
                <div id="search-results">
                    ...
                    if (isset($_GET['query'])) {
                        ...
                        if ($query !== "") {
                            $stmt = $conn->prepare("SELECT * FROM commentaries WHERE title LIKE ? OR subtitle LIKE ? OR content LIKE ? OR authors_firstname LIKE ? OR authors_lastname LIKE ? ORDER BY id DESC LIMIT 5");
                            $searchTerm = "%" . $query . "%";
                            ...
                            if ($stmt->execute()) {
                                ...
                                if ($result->num_rows > 0) {
                                    ...
                                    <div class='posts_delete_edit'>
                                        ...
                                        <a class='users_delete' onclick='confirmDeleteC2(" . $row['id'] . ")'>
                                            <i class='fa fa-trash' aria-hidden='true'></i>
                                        </a>
                                        <form id='$formId' class='favouriteForm' action='../script.php' method='POST' data-id='" . $row['id'] . "'>
                                            <input type='hidden' name='post_id5' value='" . $row['id'] . "'>
                                            <input type='hidden' name='isfavourite5' value='" . ($row['is_favourite'] === 'True' ? 'True' : 'False') . "'>
                ...
                <?php
                $select_commentaries = "SELECT id, admin_id, editor_id, title, time, DATE_FORMAT(Date, '%M %d, %Y') as formatted_date, authors_firstname, authors_lastname, is_favourite FROM commentaries ORDER BY id DESC LIMIT 100";
