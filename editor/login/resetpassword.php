...
if (isset($_REQUEST['reset_pswd'])){
    ...
    if ($password == $confirm_password){
        $reset_query = mysqli_query($conn, "UPDATE editor SET password = '$password' WHERE email = '$email'");
        if($reset_query > 0){
...
if ($_GET['secret']){
    ...
    $check_details = mysqli_query($conn, "SELECT email FROM editor WHERE email = '$email'");
