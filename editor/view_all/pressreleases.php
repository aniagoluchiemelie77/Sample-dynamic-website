    ...
    <section class="sectioneer">
        <div class="posts_div1 postsdiv sectioneer_divcontainer">
            <div class="page_links">
                <a href="../editor_homepage.php"><?php echo $translations['home']; ?></a> > <p><?php echo $translations['posts']; ?></p> > <p><?php echo $translations['view_press_releases2']; ?></p>
            </div>
            ...
            <div class="posts_divcontainer border-gradient-side-dark">
                <div id="search-results">
                    ...
                    if (isset($_GET['query'])) {
                        ...
                        if ($query !== "") {
                            $stmt = $conn->prepare("SELECT * FROM press_releases WHERE title LIKE ? OR subtitle LIKE ? OR content LIKE ? OR authors_firstname LIKE ? OR authors_lastname LIKE ? ORDER BY id DESC LIMIT 5");
                            $searchTerm = "%" . $query . "%";
                            ...
                            if ($stmt->execute()) {
                                ...
                                if ($result->num_rows > 0) {
                                    ...
                                    <div class='posts_delete_edit'>
                                        <a class='users_edit' href='../edit/post.php?id6=" . $row["id"] . "&title=" . $row["title"] . "'>
                                            <i class='fa fa-pencil' aria-hidden='true'></i>
                                        </a>
                                        <a class='users_delete' onclick='confirmDeletePR2(" . $row['id'] . ")'>
                                            <i class='fa fa-trash' aria-hidden='true'></i>
                                        </a>
                                        <form id='$formId' class='favouriteForm' action='../script.php' method='POST' data-id='" . $row['id'] . "'>
                                            <input type='hidden' name='post_id3' value='" . $row['id'] . "'>
                                            <input type='hidden' name='isfavourite3' value='" . ($row['is_favourite'] === 'True' ? 'True' : 'False') . "'>
                ...
                <?php
                $select_pressreleases = "SELECT id, admin_id, editor_id, title, time, DATE_FORMAT(Date, '%M %d, %Y') as formatted_date, authors_firstname, authors_lastname, is_favourite FROM press_releases ORDER BY id DESC LIMIT 100";
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            allForms.forEach(function(form) {
                ...
                const favInput = form.querySelector('input[name="isfavourite3"]');
