    ...
    function renderFrontendPage($ucPageTitle, $lcPageTitle)
    {
        ...
        echo '<div class="body_container">
            <div class="body_left">
                ...
                <form class="header_searchbar2 search_input" id="search_form" action="" method="get">
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if ($post_type_dbname !== 'unpublished_articles') {
                            $output .= "<form id='" . $formId . "' class='favouriteForm' action='../script.php' method='POST' data-id='" . $row['id'] . "'>
...
if (!function_exists('renderResourcesQueryResults')) {
    function renderResourcesQueryResults($query)
    {
        global $conn, $resourceType;
        $output = '';
        $stmt = $conn->prepare("SELECT * FROM $resourceType WHERE title LIKE ?");
        $searchTerm = "%" . $query . "%";
        $stmt->bind_param("s", $searchTerm);
        if ($stmt->execute()) {
            $result = $stmt->get_result();
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    $title = substr($title, 0, $max_length) . '...';
                    $output .= " <a class='more_posts_subdiv'>";
                    $output .= "<img src='../images/resurces_img.png' alt='$resourceType Image'/>";
                    $output .= " <div class='more_posts_subdiv_subdiv'>
                                    <h1>" . $title . "</h1>
                                    <span>" . $formattedDate . "</span>
                                </div>";
                    $output .= "<div class='view_whitepaper'>
                                    <div class='posts_btn' onclick=\"window.location.href='$resourcePath'\" target='_blank'>
                                        <i class='fa fa-eye' aria-hidden='true'></i>
                                    </div>
                                </div>";
                    $output .= "<p class='posts_div_niche'>" . $niche . "</p>";
                    $output .= "</a>";
                }
            } else {
                $output .= "<h1 class='bodyleft_header3'>No results found for ' $query '</h1>";
            }
        }
        return $output;
    }
}
if (!function_exists('renderResourcesFrontendPage')) {
    function renderResourcesFrontendPage($resourceNameUc, $resourceType)
    {
        global $conn, $logo;
        require("../includes/header2.php");
        echo '<div class="body_container">
                <div class="body_left">
                    <div class="page_links">
                        <a href="../">Home</a> > <p>Resources (' . $resourceNameUc . ')</p>
                    </div>
                    <h1 class="bodyleft_header3">Search ' . $resourceNameUc . '</h1>
                    <form class="header_searchbar2 search_input" id="search_form" action="" method="get">
                        <input type="text" name="query" id="search-bar" placeholder="Search.." />
                        <button class="fa fa-search" type="submit"></button>
                    </form>
        ...
        if (isset($_GET['query'])) {
            $query = trim($_GET['query']);
            echo renderResourcesQueryResults($query);
        }
        echo '</div></div>
                    <div class="more_posts">
        ';
        $sql = "SELECT name, resource_path, niche, title, date_added FROM $resourceType ORDER BY id DESC";
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<p>No " . $resourceNameUc . " Uploaded Yet.</p>";
        }
        echo '  </div></div>
                <div class="body_right border-gradient-leftside--lightdark">
                    <div class="subscribe_container">
                    ...
                    <h3 class="bodyleft_header3 border-gradient-bottom--lightdark">Editors Picks</h3>
            ';
        include("../helpers/editorspicks.php");
        echo '</div></div>';
        include("../includes/footer2.php");
